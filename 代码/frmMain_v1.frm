...
'--------------------------------------------------
' ファイル一覧更新（統合版 - RefreshFileList + SafeRefreshFileList）
' 説明：FSO を使用した完全な Unicode 対応＋エラーハンドリング強化版
'--------------------------------------------------
...
Private Sub RefreshFileList()
    ...
    ' FSO でファイルを列挙（Unicode 完全対応）
    Dim fileObj As Object
    For Each fileObj In folderObj.files
        On Error Resume Next  ' 個別ファイルのエラーをスキップ
        
        Dim ext As String
        ext = LCase$(fso.GetExtensionName(fileObj.Name))
        
        ' Excelファイルのみ処理（xls, xlsx, xlsm）
        If ext = "xls" Or ext = "xlsx" Or ext = "xlsm" Then
            ...
            If Len(filterPeriod) > 0 Then
                On Error GoTo ErrHandler  ' IsFileInPeriod でエラーが出たら全体エラーへ
            ...
            ' リストに追加（列0=ファイル名、列1=ファイル名（表示用）、列2=サイズ、列3=更新日時）
...
ErrHandler:
    ...
    lstFiles.Clear
    Set m_filePathsList = CreateObject("Scripting.Dictionary")
    lblCount.Caption = "選択件数：0 件（読込エラー: " & Err.Description & "）"
    chkSelectAll.value = False
    Debug.Print "RefreshFileList Error: " & Err.Description & " at line " & Erl
End Sub
